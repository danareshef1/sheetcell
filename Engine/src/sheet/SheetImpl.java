...
public class SheetImpl implements Sheet {
    ...
    public sheet.SheetImpl updateCellValueAndCalculate(int row, int column, String value, boolean first) {
        Coordinate coordinate = CoordinateFactory.createCoordinate(row, column);
        sheet.SheetImpl newSheetVersion = this;
        if(!first) {
    ...
    private void handleEmptyCell(Coordinate coordinate, String value, sheet.SheetImpl sheet) {
        if(value.isEmpty()){
            removeCoordinate(coordinate, sheet);
        }
    }

    public void updateOrCreateCell(Coordinate coordinate, int row, int column, String value, sheet.SheetImpl newSheetVersion
            ,boolean first) {
    ...
    private void removeCoordinate(Coordinate coordinate, sheet.SheetImpl sheet) {
        Cell cell = activeCells.get(coordinate);
    ...
    @Override
    public sheet.SheetImpl copySheet() {
        try {
            ...
            return (sheet.SheetImpl) ois.readObject();
        } catch (Exception e) {
